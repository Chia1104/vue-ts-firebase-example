...
onMounted(async () => {
  ...
  if (banners.value.listUrl.length === 0)
    await store.dispatch("getBannerAction", { category: "banner" });
});
</script>
...
<template>
  <div class="c-container">
    <main class="main w-full text-center">
      <Banner v-if="banners.isSuccess" :list-url="banners.listUrl" />
      <h1 class="text-4xl font-bold self-start mb-20">New Products</h1>
      <ProductsList
      ...
      <div class="mt-10">
        <router-link
          ...
          <span
            class="c-button-secondary transform group-hover:-translate-x-1 group-hover:-translate-y-1 text-base">
            See More
          </span>
        </router-link>
      </div>
