...
export const addProductToCart = (state: cartStateType, payload: Cart) => {
    ...
    if (product) {
        ...
        state.count = cartItems.map(i => i.productQuantity).reduce((a, b) => a + b, 0);
        state.total = cartItems.map(i => i.productQuantity * i.productPrice).reduce((a, b) => a + b, 0);
        return;
    }
    state.data.push(payload);
    state.count = state.data.map(i => i.productQuantity).reduce((a, b) => a + b, 0);
    state.total = state.data.map(i => i.productQuantity * i.productPrice).reduce((a, b) => a + b, 0);
}
