...
const editName = () => store.dispatch('updateProfileAction', { displayName: nameModal.value });

</script>

<template>
  <div class="w-full rounded-xl flex flex-col c-bg-secondary shadow-lg md:w-[75%] lg:w-[65%] group duration-300 transition ease-in-out relative">
    ...
    <div class="flex flex-col justify-center items-center z-30 absolute w-full top-[13rem]">
      <img class="rounded-full h-[150px] w-[150px] md:h-[200px] md:w-[200px] border-8 border-white" src="../../../../src/assets/profile-2.jpg" alt="profile"/>
      <AntPopover
          placement="bottom"
      >
        ...
        <template #content>
          <form
              ...
              class="w-full flex">
            <input type="text" class="w-full border-2 border-gray-300 rounded-lg p-1 mr-2" v-model="nameModal" required/>
            <button
                type="submit"
    ...
    <div class="flex flex-col w-full min-h-[350px] pt-32">
      <p class="mt-auto mb-10 text-start px-7 text-xl line-clamp-1">{{ email }}</p>
    </div>
  </div>
</template>
