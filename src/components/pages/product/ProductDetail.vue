...
const Modal = defineAsyncComponent(
  () => import("@chia/components/globals/Modal.vue")
);
...
<template>
  <article class="w-full">
    <div
      class="w-full max-w-[1400px] mx-auto grid grid-cols-1 md:grid-cols-2 gap-14">
      <div
        class="rounded-xl aspect-w-1 aspect-h-1 overflow-hidden shadow-2xl hover:cursor-zoom-in group"
        @click="handleOpen">
        <img
          :src="product.img_src || '../../../../src/assets/profile-1.jpg'"
          :alt="product.name"
          class="object-cover group-hover:scale-[1.05] transition ease-in-out duration-500"
          loading="lazy" />
      </div>
      <div>
  ...
  <Modal :is-open="localState.isOpen" :handle-close="handleClose">
