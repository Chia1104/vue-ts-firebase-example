...
export const getImage = async (imageUrl: string): Promise<string> => {
    try {
        return await getDownloadURL(ref(storage, `gs://vue-auth-example-b8ad7.appspot.com/images/${imageUrl}`))
    } catch (e) {
...
export const getListImageUrl = async (category: string) => {
    try {
        const listRef = ref(storage, `gs://vue-auth-example-b8ad7.appspot.com/images/${category}`);
        const l = await list(listRef, { maxResults: 10 });
        const urlPromises = l.items.map(item => getImage(`${category}/${item.name}`));
